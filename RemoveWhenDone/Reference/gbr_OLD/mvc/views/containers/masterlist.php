<?php
    ...
    // $db->sql('SELECT * FROM containers '. $limit);
    // $response = 
    ...
    if($last != 1){
        ...
        $paginationCtrls .= '<li><a href="'.HTTP.HTTPURL.'/mvc/?controller=container&action=masterlist&pn=1">First</a></li>';

        if ($pagenum > 1) {
            $previous = $pagenum - 1;
            $paginationCtrls .= '<li><a href="'.HTTP.HTTPURL.'/mvc/?controller=container&action=masterlist&pn='.$previous.'">Previous</a></li>';
            // Render clickable number links that should appear on the left of the target page number
            for($i = $pagenum-2; $i < $pagenum; $i++){
                if($i > 0){
                    $paginationCtrls .= '<li><a href="'.HTTP.HTTPURL.'/mvc/?controller=container&action=masterlist&pn='.$i.'">'.$i.'</a></li>';
                }
            }
        }

        // Render the target page number, but without it being a link
        $paginationCtrls .= '<li class="active"><a href="'.HTTP.HTTPURL.'/mvc/?controller=container&action=masterlist&pn='.$pagenum.'">'.$pagenum.'</a></li>';
        ...
        if ($pagenum != $last) {
            $next = $pagenum + 1;
            $paginationCtrls .= '<li><a href="'.HTTP.HTTPURL.'/mvc/?controller=container&action=masterlist&pn='.$next.'">Next</a></li>';
        }

        $paginationCtrls .= '<li><a href="'.HTTP.HTTPURL.'/mvc/?controller=container&action=masterlist&pn='.$last.'">Last</a></li>';
    }
